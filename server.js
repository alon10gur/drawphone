...
io.on('connection', (socket) => {
  ...
  function startDrawingRound(roomCode) {
    ...
    const currentPlayer = players[room.currentRound % players.length];
    ...
    io.to(roomCode).emit('round-start', {
      ...
      drawingPlayer: currentPlayer.name,
  ...
  socket.on('get-reveal-data', (roomCode, callback) => {
    ...
    const revealData = room.images.map((img, index) => {
      ...
      const drawing = room.drawings.find((d) => d.round === index);
      return {
        original: img.data,
        originalPlayer: player ? player.name : 'Unknown',
        recreation: drawing ? drawing.data : null,
        drawingPlayer: drawing
          ? room.players.get(drawing.playerId)?.name || 'Unknown'
          : null,
      };
    });
